...
import softSoundPath from "/src/assets/softMusic.mp3";
  ...
  const rewards = [
    {
      ...
      value: "x500",
      icon: "🪙",
      background: "from-purple-600/80 to-purple-800/80",
    },
    {
      type: "heart",
      value: "",
      icon: "💜",
      background: "from-purple-600/80 to-teal-600/80",
    },
    {
      type: "multiplier",
      value: "x25",
    ...
    <div className="flex flex-col items-center xl:justify-center min-h-screen bg-[#181E1A] p-4">
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50">
          ...
          <div className="modal-content relative flex flex-col px-8 py-6 rounded-2xl text-center items-center justify-center">
            ...
            <p className="text-xl text-white">
              You won {prize?.value} {prize?.icon}
            </p>
            <button
              onClick={() => setShowModal(false)}
              className="mt-6 px-6 py-2 bg-purple-600 rounded-full hover:bg-purple-700 transition-colors text-white"
            >
              Claim Reward
            </button>
          </div>
      ...
      <div className="relative w-full max-w-md">
        <div className="text-center mb-[5.5rem]">
          <h2 className="text-transparent bg-clip-text bg-gradient-to-r from-[#8655FA] to-[#E657FB] 0 text-2xl font-medium mb-4">
            Free spin every day
          </h2>
          {hasSpun && (
            <p className="text-white text-3xl font-bold">
              You have already used your Free Spin
            </p>
          )}
        </div>
        ...
        <div className="wheel-section relative w-80 h-80 mx-auto shadow-md">
          ...
          >
            {rewards.map((reward, index) => (
              <div
                key={index}
                className={`wheel-segment shadow-md ${
                  index % 2 === 0 ? "even" : ""
                }`}
                style={{
        ...
        <div className="flex items-center justify-center mt-8 text-gray-400">
          ...
          <button
            className={`w-12 h-6 rounded-full transition-colors ${
              skipAnimation ? "bg-purple-600" : "bg-gray-700"
            }`}
        ...
        <div className="mt-8 text-center">
          <p className="text-gray-400 mb-6 text-base px-[2rem] sm:px-0">
